...
FULL_ORDER = ["hotfix-nvfp4-ds-mla-issue22.sh"] + SEVEN  # 3x: spin-wait not wired

# 3x adaptation: this recipe vendors hotfix-gb10-spin-wait.sh but does NOT
# invoke it (TP=2-specific; deferred pending a 3x mesh A/B), so there is no
# DSPARK_SKIP_SPIN_WAIT_HOTFIX gate in docker-compose.yml.
SKIP_VARS = (
    "DSPARK_SKIP_HOTFIX",
...
def compose_fail_closed_blocks():
    """3x-native extraction: pull the two hotfix-wiring blocks from
    docker-compose.yml — the issue22 gate line and the seven-script loop —
    each exactly once, de-$$-escaped so they can run standalone."""
    text = COMPOSE.read_text()
    lines = text.splitlines()

    # issue22: single ENTRYPOINT gate line (the $${...} one, not the
    # environment: passthrough)
    i22 = [
        ln.strip() for ln in lines
        if "DSPARK_SKIP_ISSUE22_HOTFIX" in ln and "$${" in ln
    ]
    if len(i22) != 1:
        raise AssertionError(f"expected exactly one issue22 gate line, got {len(i22)}")

    # loop: from the DSPARK_SKIP_HOTFIX if-line through its closing fi;
    start = None
    for idx, ln in enumerate(lines):
        if "DSPARK_SKIP_HOTFIX:-0" in ln and "for _hf in" not in ln and "$${" in ln:
            start = idx
            break
    if start is None:
        raise AssertionError("loop gate if-line not found")
    end = None
    for idx in range(start, len(lines)):
        if lines[idx].strip() == "fi;":
            ...
            break
    if end is None:
        raise AssertionError("loop closing fi; not found")
    loop = [ln.strip() for ln in lines[start:end + 1]]

    def de(ss):
        return [s.replace("$$", "$") for s in ss]

    return de(i22), de(loop)


ISSUE22 = "hotfix-nvfp4-ds-mla-issue22.sh"


class ComposeFailClosedWiring(unittest.TestCase):
    """Fail-closed wiring of the 3x compose hotfix blocks: default order,
    per-group skip switches, missing-script FATAL, nonzero exit blocks exec."""

    @classmethod
    def setUpClass(cls):
        cls.block_i22, cls.block_loop = compose_fail_closed_blocks()

    def run_blocks(self, env_extra=None, stub_exits=None, remove=()):
        workdir = Path(tempfile.mkdtemp(prefix="compose-wiring-3x-"))
        self.addCleanup(shutil.rmtree, workdir)
        patches = workdir / "dspark-patches"
        patches.mkdir()
        exits = dict(stub_exits or {})
        for name in [ISSUE22] + SEVEN:
            if name in remove:
                continue
            stub = patches / name
        ...
        script = (
            "\n".join(self.block_i22 + self.block_loop)
            .replace("/opt/dspark-patches", str(patches))
            + f'\nprintf x >> "{reached}"\n'
        )
        env = dict(os.environ)
        for var in SKIP_VARS:
    ...
    def test_default_boot_runs_all_shell_hotfixes_in_compose_order(self):
        r, inv, reached = self.run_blocks()
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
        got = inv.read_text().split()
        self.assertEqual(got, [ISSUE22] + SEVEN)
        self.assertTrue(reached.exists())

    def test_each_skip_switch_skips_only_its_group(self):
        # issue22 skip: only issue22 is skipped, the loop still runs
        r, inv, reached = self.run_blocks(env_extra={"DSPARK_SKIP_ISSUE22_HOTFIX": "1"})
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
        self.assertEqual(inv.read_text().split(), SEVEN)
        # whole-train skip: only issue22 runs
        r, inv, reached = self.run_blocks(env_extra={"DSPARK_SKIP_HOTFIX": "1"})
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
        self.assertEqual(inv.read_text().split(), [ISSUE22])

    def test_missing_enabled_script_fails_closed_before_exec(self):
        for missing in [ISSUE22, SEVEN[0], SEVEN[-1]]:
            r, inv, reached = self.run_blocks(remove=(missing,))
            self.assertNotEqual(r.returncode, 0, f"{missing}: expected failure")
            self.assertIn("FATAL", r.stdout + r.stderr)
            self.assertIn(missing, r.stdout + r.stderr)
            self.assertFalse(reached.exists(), "exec must not be reached")

    def test_nonzero_script_exit_blocks_exec(self):
        for script, code in [(ISSUE22, 7), (SEVEN[3], 7)]:
            r, inv, reached = self.run_blocks(stub_exits={script: code})
            self.assertNotEqual(r.returncode, 0, f"{script}: expected failure")
            self.assertFalse(reached.exists(), "exec must not be reached")

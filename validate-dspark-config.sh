...
# Match start.sh: text profile only on the 3x (no vision sidecar).
...
# 3x topology: three NODE vars instead of the 2x WORKER_HOST
: "${NODE0_HOST:?NODE0_HOST must be set in $ENV_FILE}"
: "${NODE1_HOST:?NODE1_HOST must be set in $ENV_FILE}"
: "${NODE2_HOST:?NODE2_HOST must be set in $ENV_FILE}"
: "${NODE0_IP:?NODE0_IP must be set in $ENV_FILE}"
: "${MASTER_PORT:?MASTER_PORT must be set in $ENV_FILE}"
: "${DSPARK_VLLM_IMAGE:?DSPARK_VLLM_IMAGE must be set in $ENV_FILE}"

# 3x: master addr is the node0 fabric IP
...
echo "  serve mode: $DSPARK_SERVE_MODE"

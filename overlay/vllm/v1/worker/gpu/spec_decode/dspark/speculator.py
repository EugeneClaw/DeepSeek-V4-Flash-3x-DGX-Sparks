...
class DSparkSpeculator(DFlashSpeculator):
    ...
    def load_draft_model(
        ...
        # DSpark is not MixtureOfExperts, so V2 EPLBController skips
        # add_model() on the draft. Routers still have an empty
        # EplbLayerState because enable_eplb=True, and then
        # _validate_eplb_state raises. Install an identity dummy so
        # logical IDs stay the primary physical replicas.
        self._install_dummy_eplb_state(model)
        return model

    def _install_dummy_eplb_state(self, model: torch.nn.Module) -> None:
        filled = 0
        device = self.device
        for mod in model.modules():
            st = getattr(mod, "eplb_state", None)
            if st is None or getattr(st, "expert_load_view", None) is not None:
                continue
            n = int(
                getattr(mod, "global_num_experts", 0)
                or getattr(mod, "num_experts", 0)
                or getattr(mod, "num_logical_experts", 0)
                or 0
            )
            if n <= 0:
                n = 256
            # Compact identity: logical i -> physical i, one replica.
            logical_to_physical = torch.full(
                (n, 2), -1, dtype=torch.int32, device=device
            )
            logical_to_physical[:, 0] = torch.arange(
                n, dtype=torch.int32, device=device
            )
            st.logical_to_physical_map = logical_to_physical
            st.logical_replica_count = torch.ones(n, dtype=torch.int64, device=device)
            st.expert_load_view = torch.zeros(n, dtype=torch.int32, device=device)
            st.should_record_tensor = torch.zeros((), dtype=torch.bool, device=device)
            st.num_unpadded_tokens_tensors = [
                torch.zeros((), dtype=torch.int32, device=device)
            ]
            filled += 1
        if filled:
            logger.info(
                "DSpark: installed identity EPLB dummy on %d draft modules "
                "(expert_load_view was None).",
                filled,
            )

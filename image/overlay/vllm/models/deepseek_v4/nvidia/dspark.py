...
def _pad_loaded_for_tp_narrow(
    param: torch.Tensor,
    loaded_weight: torch.Tensor,
    tp_size: int,
    name: str = "",
) -> torch.Tensor:
    """Zero-pad checkpoint dims so TP narrow(rank*local, local) fits.

    Parallel loaders compute shard size from the (padded) param, then
    narrow the (unpadded) checkpoint tensor. Pad a dim only when
    ``local < have < tp*local`` so replicated / already-padded tensors
    are left alone (a full copy_ would break if we over-pad).
    """
    if tp_size <= 1 or not hasattr(loaded_weight, "shape") or loaded_weight.ndim == 0:
        return loaded_weight
    # VocabParallelEmbedding asserts loaded dim == org_vocab_size, then
    # pads internally. org_vocab_size lives on the *module*, not the
    # Parameter, so detect by name. 129280 < 129408 matches the pad rule.
    if getattr(param, "org_vocab_size", None) is not None:
        return loaded_weight
    if name and any(m in name for m in _VOCAB_NAME_MARKERS):
        return loaded_weight
    out = loaded_weight
    nd = min(out.ndim, param.ndim)
    for dim in range(nd):
        local = int(param.shape[dim])
        have = int(out.shape[dim])
        need = tp_size * local
        if not (0 < local < have < need):
            continue
        pad_shape = list(out.shape)
        pad_shape[dim] = need - have
        out = torch.cat([out, out.new_zeros(pad_shape)], dim=dim)
    return out
...
class DSparkDeepseekV4Model(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # Still constructed here, so TP=3 needs the same vocab pad quantum.
        from vllm.distributed import get_tensor_model_parallel_world_size
        from vllm.models.deepseek_v4.tp_pad import vocab_padding_size
        ...
        self.embed_tokens = VocabParallelEmbedding(
            ...
            padding_size=vocab_padding_size(_tp),
        )
...
class DSparkDeepseekV4ForCausalLM(nn.Module):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        from vllm.models.deepseek_v4.tp_pad import (
            apply_config_tp_pad,
            orig_groups,
            orig_heads,
            orig_moe_intermediate,
            orig_routed_experts,
            pad_loaded_weight_for_attn,
            pad_loaded_weight_for_moe,
        )

        apply_config_tp_pad(self.config, tp_size)
        _pad_attn = getattr(self.config, "_dsv4_tp_pad_applied", False)
        _orig_h = orig_heads(self.config)
        _orig_g = orig_groups(self.config)
        _orig_e = orig_routed_experts(self.config)
        _orig_i = orig_moe_intermediate(self.config)
        _pad_h = int(self.config.num_attention_heads)
        _pad_g = int(self.config.o_groups)
        _pad_e = int(self.config.n_routed_experts)
        _pad_i = int(self.config.moe_intermediate_size)
        _head_dim = int(self.config.head_dim)
        _o_lora = int(self.config.o_lora_rank)

        for name, loaded_weight in weights:
            mapped = self._remap_dspark_name(name)
            if mapped is None:
                continue
            if _pad_attn and "attn" in mapped:
                loaded_weight = pad_loaded_weight_for_attn(
                    mapped,
                    loaded_weight,
                    head_dim=_head_dim,
                    o_lora_rank=_o_lora,
                    padded_heads=_pad_h,
                    padded_groups=_pad_g,
                    orig_heads=_orig_h,
                    orig_groups=_orig_g,
                )
            if _pad_attn and (
                "ffn.gate" in mapped
                or "e_score_correction_bias" in mapped
                or ".experts." in mapped
            ):
                loaded_weight = pad_loaded_weight_for_moe(
                    mapped,
                    loaded_weight,
                    padded_experts=_pad_e,
                    orig_experts=_orig_e,
                    padded_intermediate=_pad_i,
                    orig_intermediate=_orig_i,
                )
            name = mapped
            ...
            for param_name, weight_name, stacked_shard_id in stacked_params_mapping:
                ...
                loaded_weight = _pad_loaded_for_tp_narrow(
                    param, loaded_weight, tp_size, name
                )
                ...
                try:
                    ...
                    logger.error(
                        "DSpark TP pad load failed name=%s loaded=%s param=%s tp=%s rank=%s",
                        name,

...
class DeepseekV4Attention(nn.Module, AttentionLayerBase, ABC):
    ...
    def __init__(
        ...
        apply_config_tp_pad(config, tp_size)

        self.prefix = prefix  # Alias for compatibility with compressor
        self.hidden_size = config.hidden_size
        self.n_heads = config.num_attention_heads
        assert self.n_heads % tp_size == 0, (
            f"n_heads={self.n_heads} not divisible by tp_size={tp_size}; "
            f"set VLLM_DSV4_TP_PAD=1 for Flash-0731 pad recipe"
        )
        ...
        assert self.n_groups % tp_size == 0, (
            f"o_groups={self.n_groups} not divisible by tp_size={tp_size}"
        )
        self.n_local_groups = self.n_groups // tp_size
        assert self.n_local_heads % self.n_local_groups == 0, (
            f"n_local_heads={self.n_local_heads} not divisible by "

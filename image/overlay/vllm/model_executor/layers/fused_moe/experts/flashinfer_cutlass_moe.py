...
class FlashInferExperts(mk.FusedMoEExpertsModular):
    ...
    def __init__(
        ...
        if quant_config.weight_quant_dtype == "mxfp4":
            # 1.702 is the GPT-OSS SwiGLU alpha. DeepSeek-V4 is standard
            # SiLU/SwiGLU (alpha=1). Prefer values from the quant config.
            routing = getattr(moe_config, "routing_method", None)
            routing_name = getattr(routing, "name", str(routing))
            is_plain_silu = any(
                tag in routing_name
                for tag in ("DeepseekV4", "DeepSeekV4", "Deepseek", "DeepSeek")
            )
            default_alpha = 1.0 if is_plain_silu else 1.702
            alpha = (
                quant_config.gemm1_alpha
                if getattr(quant_config, "gemm1_alpha", None) is not None
                else default_alpha
            )
            beta = (
                quant_config.gemm1_beta
                if getattr(quant_config, "gemm1_beta", None) is not None
                else 1.0
            )
            # FlashInfer 0.6.15: passing ANY of swiglu_alpha/beta/limit
            # switches ActivationType.Swiglu → SwigluBias (GPT-OSS).
            # One-expert oracle on SuperDeepseek-0731: swap+interleave +
            # plain SiLU cosine=0.999; same convert + α/limit tensors
            # cosine=0.87 (garbage after 43 layers).
            if float(alpha) == 1.0:
                ...
                self.gemm1_alpha = torch.tensor(
                    [float(alpha)] * self.num_experts,
                    dtype=torch.float32,
                    device=self.device,
                )
                self.gemm1_beta = torch.tensor(
                    [float(beta)] * self.num_experts,
                    dtype=torch.float32,
                    device=self.device,
                )
    ...
    def apply(
        ...
        if (
            ...
            # gemm1_alpha/beta/limit stay None for DeepSeek plain SiLU.

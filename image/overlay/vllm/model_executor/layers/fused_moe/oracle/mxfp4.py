...
def convert_weight_to_mxfp4_moe_kernel_format(
    ...
    if mxfp4_backend in TRTLLM_BACKENDS:
        ...
        # DeepSeek stores fused gate/up as contiguous [w1; w3]. FlashInfer
        # CUTLASS wants [w3; w1], then MXFP8 block-scale interleave.
        # Do not use the GPT-OSS even/odd row de-interleave.
        def _swap_w13_to_w31(t: torch.Tensor) -> torch.Tensor:
            w1, w3 = torch.chunk(t, 2, dim=1)
            return torch.cat([w3, w1], dim=1)

        w13_weight = _swap_w13_to_w31(w13_weight.data)
        w13_weight_scale = _swap_w13_to_w31(w13_weight_scale.data)
        if w13_bias is not None:
        ...
        raise ValueError(
            ...
            f"Expected TRTLLM, Triton, AITER, XPU, or FlashInfer CUTLASS."
        )
